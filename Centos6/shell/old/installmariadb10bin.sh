...
# 建立数据库安装路径，并授权
mkdir -p $mysql_install_dir_real
chown mysql.mysql -R $mysql_install_dir_real
ln -s $mysql_install_dir_real $mysql_install_dir
# 建立数据库数据存放路径，并授权
...
# 安装jemalloc依赖包，未打包进二进制安装包中
yum -y localinstall bin/${jemalloc_rpm}
yum -y localinstall bin/${jemalloc_devel_rpm}

# 安装MariaDB二进制包
tar zxf bin/${mariadb_name}.tar.gz
rm -rf ${mysql_install_dir}/*
mv ${mariadb_name}/* ${mysql_install_dir} 
rm -rf ${mariadb_name}

# 启用jemalloc内存管理
cp $mysql_install_dir/bin/mysqld_safe{,_bk} -n
...
# 添加lib库自动搜索路径到/etc/ld.so.conf
rm -rf /etc/ld.so.conf.d/{mysql,mariadb,percona}*.conf
echo "$mysql_install_dir/lib" > /etc/ld.so.conf.d/mariadb.conf 
